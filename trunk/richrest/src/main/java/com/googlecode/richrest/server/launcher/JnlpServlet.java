package com.googlecode.richrest.server.launcher;
...
public class JnlpServlet extends HttpServlet {
	...
	public void init() throws ServletException {
					...
					if (dirFile.exists() && dirFile.isDirectory()) {
						...
						if (files != null && files.length > 0) {
							for (File file : files) {
								...
								if (mainJar == null
										&& file.getName().startsWith("richrest-client")) {
									mainJar = name;
								} else {
									jars.add(name);
								}
		...
		if (jars.size() == 0)
			throw new ServletException("The \"jars\" param is required in JnlpServlet!");
		if (mainJar == null)
			throw new ServletException("No such richrest-client-x.x.jar!");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		writer.write("	<application-desc main-class=\"com.googlecode.richrest.client.launcher.Main\">\n");
